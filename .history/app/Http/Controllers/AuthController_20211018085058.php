...
class AuthController extends Controller
{
   ...
   public function AddAbsenceDate(AddAbsenceRequest $request){
    $request->validated();
    $listOfDates[] = $this->absence->getAbsenceDates();

    if(in_array($request->absence_date	, $listOfDates)){
        return $this->response->returnError('You are in this day!', 403);

      /*  $update = $this->absence->where('id' , )->create([
            'user_id' => auth()->user()->id,
    ...
    return $this->response->returnError($listOfDates, 403);
   }
