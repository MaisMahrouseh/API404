...
class AuthController extends Controller
{
    ...
    public function AddlogoutTimeYesterday(Request $request){
       ...
       if( Carbon::parse($LastID->login_date_time)->toTimeString() > $time )
           return $this->response->returnError('The time must be greater than'. $time  , 403);

         $this->entrie->where('id' , $LastID->id)->update([
           'logout_date_time' => $day.' '.$time,
           'status' => '0'
       ]);


       $out =  $this->entrie->logoutTimeYesterday($request);
       if($out)
       return  $this->response->returnSuccess('You have been successfully logged out!' , 200);
    return $this->response->returnError('Exit error' , 500);
   }
